...
class DatabaseTest(IntegrationTest):
    ...
    def test_add_guild_new_guild(self):
        self.db.add_guild(self.new_guild.id, self.new_guild.music_channel_id)

        guild_db = self.session.query(Guild).filter(Guild.id == self.new_guild.id).first()
        guild_cached = self.db.get_guild_by_id(self.new_guild.id)
    ...
    def test_add_guild_existing_guild(self):
        with self.assertRaises(sqlalchemy.exc.IntegrityError):
            self.db.add_guild(self.existing_guild.id, self.existing_guild.music_channel_id)
            self.session.flush()
    ...
    def test_update_guild(self):
        ...
        guild_cached = self.db.get_guild_by_id(self.existing_guild.id)
    ...
    def test_get_artist_by_id(self):
        ...
        self.assertEqual(self.existing_artist.name, returned_artist.name)
    ...
    def test_add_artist_new_artist(self):
        ...
        artist_cached = self.db.get_artist_by_id(self.new_artist.id)
        guild_cached = self.db.get_guild_by_id(self.existing_guild.id)

        self.assertEqual(self.new_artist.name, added_artist.name)
        self.assertEqual(added_artist.name, artist_cached.name)
        self.assertTrue(self.new_artist.id in guild_cached.artist_ids)
    ...
    def test_unfollow_artist_for_guild_artist_still_exists(self):
        ...
        existing_guild_db = self.session.query(Guild).filter(Guild.id == self.existing_guild.id).first()
        existing_guild_cached = self.db.get_guild_by_id(self.existing_guild.id)
        new_guild_cached = self.db.get_guild_by_id(self.new_guild.id)
        artist_cached = self.db.get_artist_by_id(self.existing_artist.id)
